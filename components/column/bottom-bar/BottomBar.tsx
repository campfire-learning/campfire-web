...
export const BottomBar = ({ isMini }: { isMini?: boolean }) => {
  ...
  useEffect(() => {
    setFirstName(
      localStorageExists && JSON.parse(localStorage.getItem("user") || "null")?.first_name
    );
    setLastName(
      localStorageExists && JSON.parse(localStorage.getItem("user") || "null")?.last_name
    );
    setRole(
      localStorageExists && startCase(JSON.parse(localStorage.getItem("user") || "null")?.user_type)
    );
  }, []);
